...
class TimeSeriesSplitterTest(unittest.TestCase):
    def test_split(self):
        """
        """
        data = pd.DataFrame.from_dict({"x": [i for i in range(0, 200)], "y": [1 for _ in range(0, 200)]})
        ...
        for train_data, test_data in splitter.split(data):
            cnt += 1
            self.assertEqual(
                test_data["x"].values.tolist(), [-1 + cnt + splitter.train_size, cnt + splitter.train_size]
            )
