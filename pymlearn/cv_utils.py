...
class CVSplitter(object):
    ...
    def split(self, data):
        ...
        for train_idx, test_idx in kf.split(data):
            yield data.loc[train_idx].reset_index(drop=True), data.loc[test_idx].reset_index(drop=True)


def train_validate(estimator, train_data, test_data, perf_score_fn=clf_perf_scores, verbose=0):
    estimator = copy.deepcopy(estimator)  # NB: must do this because estimator is mutable
    estimator.fit(train_data, verbose=verbose)
    y_pred = estimator.predict(test_data)
    y_true = test_data[estimator.get_ycol()]

    y = pd.DataFrame.from_dict({"true": y_true, "pred_cat": y_pred["pred_cat"].values})
...
def cross_validate(data, estimator, cv_splitter, perf_score_fn=clf_perf_scores, verbose=0):
    ...
    for train_data, test_data in cv_splitter.split(data):
        ...
        _, perf_row = train_validate(estimator, train_data, test_data, perf_score_fn=perf_score_fn, verbose=verbose)
